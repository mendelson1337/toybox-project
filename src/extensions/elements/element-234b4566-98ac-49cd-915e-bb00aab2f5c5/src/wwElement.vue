    ...
    setup(props, { emit }) {
        ...
        function handleCurrencyKeydown(event) {
            ...
            if (event.key === '-') {
                ...
                // Only allow minus at position 0 and if not already present
        ...
        function handleCurrencyBlur(event) {
            ...
            // Handle negative values in blur formatting
            const isNegative = rawValue.startsWith('-');
            let valueToFormat = isNegative ? rawValue.substring(1) : rawValue;

            // Apply final formatting with zero padding on blur
            if (valueToFormat && decimalPlaces > 0) {
                const parts = valueToFormat.split(decimalSep);
                ...
                let finalValue = integerPart + decimalSep + decimalPart;
        ...
        function handleCurrencyInput(event) {
            ...
            if (thousandsSep === decimalSep) {
                ...
                return; // Don't process if separators are invalid
            }

            // Extract negative sign if present
            const isNegative = rawValue.startsWith('-');
            let cleanValue = isNegative ? rawValue.substring(1) : rawValue;
            let hasUserDecimal = false;

            // Remove any invalid characters - keep only digits and common separators
            ...
            let actualValue = limitedCleanValue === '' ? '' : (parseFloat(limitedCleanValue) || 0);
            
            // Handle negative values
            if (isNegative && actualValue !== 0 && actualValue !== '') {
            ...
            if (cursorPosition >= rawValue.length) {
                ...
                const digitsAndSpecialBeforeCursor = beforeCursor.replace(
                ...
                for (let i = 0; i < formattedValue.length; i++) {
                    ...
                    if (/\d/.test(char) || char === decimalSep || char === '-') {
                        if (count < digitsAndSpecialBeforeCursor.length) {
                            count++;
        ...
        watch(
            ...
            () => {
                if (props.content.type === 'currency' && currencyDisplayValue.value) {
                    ...
                    if (isNegative) {
                        cleanValueStr = Math.abs(numericValue).toString();
                    }
