...
export function useCurrency(props, { variableValue } = {}) {
    ...
    const formatCurrency = (value, options = {}) => {
        ...
        // Remove currency symbol if present from string representation
        let inputVal = absoluteValue.toString();
        ...
        if (includeSymbol && showCurrencySymbol.value && currencySymbol.value) {
            if (symbolPosition.value === 'prefix') {
                // For negative values with prefix symbol: -$123.45
                if (isNegative) {
                    formattedValue = '-' + currencySymbol.value + formattedValue.substring(1);
                } else {
                    formattedValue = currencySymbol.value + formattedValue;
                }
            } else {
                // For negative values with suffix symbol: -123.45$
