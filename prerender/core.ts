    ...
    return (
        ...
        value.every(
            item =>
                typeof item === 'string' &&
                item.length > 0 &&
                item.length <= MAX_CLIENT_ISLAND_ID_LENGTH
        )
    );
}
...
export function enrichRouteHtml(
    ...
    return htmlWithStyles.replace(
        APP_MOUNT_POINT,
        `<div id="app" data-ww-prerendered="true" data-ww-client-islands="${serializedClientIslands}">${appHtml}</div>`
    );
}
...
function getBuildAssetPrefix(html: string): string {
